function [U,V,NX,NY,detadx,d2pxi2,d2pxin] = sderivatives2BODY(nodexbody,nodeybody,x,y,p,dpdn,qnode,corner6)
...
pf = p(C2+1:C3-1); qf = dpdn(C2+1:C3-1); % get fs with double nodes


%% Prefer to keep odd number of nodes on the free surface  
M = length(nodexbody)/2-0.5;
...
for M = 1:M
    for n = 1:3
            xi  = zeta(n);
            xnode(M,n) = -(xi/2)*(1-xi)*nodexbody(fnode(M,n)) +  (1+xi)*(1-xi)*nodexbody(fnode(M,n)) + (xi/2)*(1+xi)*nodexbody(fnode(M,n)) ;
            ynode(M,n) = -(xi/2)*(1-xi)*nodeybody(fnode(M,n)) +  (1+xi)*(1-xi)*nodeybody(fnode(M,n)) + (xi/2)*(1+xi)*nodeybody(fnode(M,n)) ;
