function [U,V,NX,NY,detadx,d2pxi2,d2pxin,dpxi] = sderivatives2BODYQ(bodynodexbody,bodynodeybody,x,y,p,dpdn,qnode,corner6)
% Body may need velocity components in intrinsic coordinates
C1 = corner6(1); C2 = corner6(2); C3 = corner6(3); C4 = corner6(4); C5 = corner6(5); C6 = corner6(6);

pf = p(C2:C3); qf = dpdn(C2:C3); % get fs with double nodes


%% Prefer to keep odd number of nodes on the free surface  
M = length(bodynodexbody)/2-0.5;
...
for M = 1:M
    for n = 1:3
            xi  = zeta(n);
            xnode(M,n) = -(xi/2)*(1-xi)*bodynodexbody(fnode(M,n)) +  (1+xi)*(1-xi)*bodynodexbody(fnode(M,n)) + (xi/2)*(1+xi)*bodynodexbody(fnode(M,n)) ;
            ynode(M,n) = -(xi/2)*(1-xi)*bodynodeybody(fnode(M,n)) +  (1+xi)*(1-xi)*bodynodeybody(fnode(M,n)) + (xi/2)*(1+xi)*bodynodeybody(fnode(M,n)) ;
...
for M = 1:M
    for n = 1:3
            ...
            dpe(M,n) = xi*(pnode(M,1) -2*pnode(M,2)+pnode(M,3))    +   (pnode(M,3) - pnode(M,1))*0.5;   % dphi_dxi
            d2pe2(M,n) = (pnode(M,1) - 2*pnode(M,2) +pnode(M,3));                                       % d2phi_dxi2
            d2pdne(M,n) = xi*(dpnode(M,1) -2*dpnode(M,2)+dpnode(M,3))    +   (dpnode(M,3) - dpnode(M,1))*0.5;
