function [U,V,NX,NY,detadx] = sderivatives2LEFT(xfl,yfl,x,y,p,dpdn,qnode,corner6)

C1 = corner6(1); C2 = corner6(2); C3 = corner6(3); C4 = corner6(4); C5 = corner6(5); C6 = corner6(6);

pf = p(1:C2); qf = dpdn(1:C2); % get fs with double nodes
...
for M = 1:M
    for n = 1:3
            xi  = zeta(n);
            xnode(M,n) = -(xi/2)*(1-xi)*xfl(fnode(M,n)) +  (1+xi)*(1-xi)*xfl(fnode(M,n)) + (xi/2)*(1+xi)*xfl(fnode(M,n)) ;
            ynode(M,n) = -(xi/2)*(1-xi)*yfl(fnode(M,n)) +  (1+xi)*(1-xi)*yfl(fnode(M,n)) + (xi/2)*(1+xi)*yfl(fnode(M,n)) ;
